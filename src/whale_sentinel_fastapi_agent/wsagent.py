    ...
    def _communication(self):
        ...
        try:
            ...
            if gateway_response is None:
                logger.info("Whale Sentinel FastAPI Agent Protection: Communication with Whale Sentinel Gateway failed")
                for i in range(3):
                    time.sleep(30)
                    logger.info(f"Whale Sentinel FastAPI Agent Protection: Attempt communication {i + 1} failed")
                    gateway_response = Agent._make_call(self, endpoint, data)
                    if gateway_response is not None:
                        break
            else:
                logger.info("Whale Sentinel FastAPI Agent Protection: Commmunication with Whale Sentinel Gateway successful")
        except Exception as e:
